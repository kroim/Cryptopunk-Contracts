...
contract PotatoPower is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(string memory _baseURII) ERC721("Potato Power", "PW") {
    ...
    function mintByAdmin(uint256 _quantity) public onlyOwner returns (uint256) {
        require(
            _quantity <= 60,
            "Your are not allowed to mint more than 60 tokens"
        );
        require(
            totalSupplyy() + _quantity <= maxSupply,
            "Quantity must be lesser then MaxSupply"
    ...
    function updateBaseURI(string memory _baseURIII)
        public
        onlyOwner
        returns (string memory)
    {
