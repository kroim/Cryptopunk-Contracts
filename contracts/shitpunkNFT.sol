...
contract shitPonk is ERC721 , Ownable {
   ...
   uint256[] addedPondsID; 
   uint256 mintIndex;
   mapping (uint256 => ponk )  public shitPonks;
   mapping(address => bool) public isMinter;
   uint256 punkMintPrice = 50000000000000000;
   ...
   constructor () ERC721("SHIT PONT NFT" , "SHITPONK"){}
    ...
    function mintPunk(uint256 amount) public payable  returns (uint256) {
        ...
        require(currentAddedPonks() > 0  && mintIndex < currentAddedPonks() , "no punks available for minting");
        require(!shitPonks[mintIndex+1].isMinted ,"next ponk minted");
        ...
        ponk storage  currentshitponk =  shitPonks[mintIndex];
        currentshitponk.creator =  _msgSender();
        currentshitponk.CurrentOwnerstartTime = block.timestamp;
        currentshitponk.isMinted = true;
       
         return currentshitponk.id;
    }
    ...
    function attributesAvailable(uint256[] memory _attributes) private returns(bool){
     ...
     function currentAddedPonks() public view returns(uint256){
  ...
  function claimReward(uint256 ponkID) public {
      uint256 balance = getReflectionBalance(ponkID);
      require(balance > 0 , "nothing to claim");
      address payable receiver = payable(ownerOf(ponkID));
      receiver.transfer(balance);
      lastDevidentAt[ponkID] = totalDevident;
  }
   ...
   function getReflectionBalance(uint256 ponkID) public view returns (uint256){
          return totalDevident - lastDevidentAt[ponkID];
   }
